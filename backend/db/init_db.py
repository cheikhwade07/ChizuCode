...
def _group_by_path(files: list[dict]) -> list[list[dict]]:
    """
    Group files by directory for frontend-heavy repos.

    Key strategy:
    - Root-level files (no directory separator) → "__root__" bucket so they
      cluster together instead of each becoming a singleton.
    - Shallow files directly inside a top-level dir (e.g. app/layout.tsx) →
      keyed to that top-level dir ("app"), not to the filename.
    - Deeper files → keyed to top-two-dir prefix (e.g. "app/dashboard").

    This prevents the common failure mode where every config/auth file at repo
    root produces its own group-of-1, yielding dozens of singleton leaf nodes.
    """
    groups: dict[str, list[dict]] = {}
    for f in files:
        parts = f["file_path"].replace("\\", "/").split("/")
        if len(parts) == 1:
            # True root-level file (e.g. "auth.ts", "package.json")
            key = "__root__"
        elif len(parts) == 2:
            # File directly inside a top-level dir (e.g. "app/layout.tsx")
            # Key to the directory only, not the filename
            key = parts[0]
        else:
            # Deeper file (e.g. "app/dashboard/page.tsx")
